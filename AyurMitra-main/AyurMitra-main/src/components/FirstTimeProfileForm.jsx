...
const FirstTimeProfileForm = ({ onComplete }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/patient/profile/create`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-emerald-100 to-green-50 flex items-center justify-center p-6">
        ...
        <div className="bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-2xl rounded-[2rem] shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-white/10 overflow-hidden relative">
          ...
          <div className="relative bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-600 p-8 text-white overflow-hidden">
            ...
            <div className="relative text-center">
              <div className="w-20 h-20 bg-white/20 backdrop-blur-md rounded-3xl flex items-center justify-center mx-auto mb-6 border border-white/30 shadow-lg">
                <User className="w-10 h-10 drop-shadow-sm" />
              </div>
              <h2 className="text-4xl font-black tracking-tight drop-shadow-sm mb-3">Welcome!</h2>
              <p className="text-white/90 font-medium text-lg drop-shadow-sm">Let's set up your profile to get started</p>
            </div>
          </div>
                ...
                <div className="relative flex items-center justify-center space-x-4">
                  {isLoading ? (
                    <>
                      ...
                      <span className="tracking-wide">Creating Profile...</span>
                    </>
                  ) : (
                    <>
                      <Save className="w-7 h-7 group-hover:scale-125 transition-all duration-300" />
                      <span className="tracking-wide">Complete Setup</span>
                    </>
                  )}
                </div>
              </button>
...
export default FirstTimeProfileForm;