...
import { Calendar, Clock, Activity, MessageSquare, Settings, User, Heart, Shield, Zap, ArrowRight, Sparkles, Star, Send } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {Array.from({ length: 5 }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  role="radio"
                >
                  <Star className={`w-6 h-6 ${active ? 'text-amber-400' : 'text-gray-300'}`} fill={active ? 'currentColor' : 'none'} />
                </button>
              );
            })}
          </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-emerald-100 flex items-center justify-between">
          <div>
            <div className="text-sm font-medium text-gray-700">Submit anonymously</div>
            <div className="text-xs text-gray-500">Your name will be hidden</div>
          </div>
          <label className="inline-flex items-center cursor-pointer">
            <input
              ...
              className="sr-only"
            />
            <span className={`w-10 h-6 flex items-center bg-emerald-200 rounded-full p-1 transition ${isAnonymous ? 'bg-emerald-500' : ''}`}>
              <span className={`bg-white w-4 h-4 rounded-full shadow transform transition ${isAnonymous ? 'translate-x-4' : ''}`}></span>
            </span>
          </label>
        </div>
      </div>
            ...
            >
              ...
              <div className="bg-gradient-to-br from-white to-emerald-50/40 rounded-3xl p-8 shadow-xl border border-emerald-200/60">
                <div className="text-center mb-8">
                  ...
                  <p className="text-slate-600">Tell us what went well and where we can improve.</p>
                </div>

                {/* Feedback form */}
                <FeedbackForm
                  onSubmit={(entry) => setFeedbackList([entry, ...feedbackList])}
                  ...
                  <div className="mt-10">
                    ...
                    <div className="space-y-4">
                      {feedbackList.slice(0, 6).map((f, i) => (
                        <div key={i} className="bg-white rounded-xl p-4 border border-emerald-100 flex items-start gap-4">
                          <div className="flex items-center">
                            {Array.from({ length: 5 }).map((_, si) => (
                              <Star key={si} className={`w-4 h-4 ${si < f.rating ? 'text-amber-400' : 'text-gray-300'}`} fill={si < f.rating ? 'currentColor' : 'none'} />
                            ))}
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-gray-800 mt-1 whitespace-pre-line">{f.comment}</p>
                          </div>
                        </div>
                      ))}
                  </div>
            ...
            >
              ...
              <div className="bg-gradient-to-br from-white to-green-50/30 rounded-3xl p-8 shadow-xl border border-green-200/50">
                <div className="text-center mb-8">
                  ...
                  <p className="text-slate-600 text-lg">Monitor your Ayurvedic wellness journey and dosha balance</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                  <div className="bg-white rounded-2xl p-6 border border-green-100 shadow-lg">
                    <div className="flex items-center mb-4">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-sm text-gray-600">Current therapy status</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium">Herbal Medicine</span>
                        <div className="flex-1 mx-3 bg-gray-200 rounded-full h-2">
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-green-100 shadow-lg">
                    <div className="flex items-center mb-4">
                      ...
                      <div className="ml-4">
                        <h3 className="text-lg font-bold text-gray-900">Symptom Tracker</h3>
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-green-100 shadow-lg">
                    <div className="flex items-center mb-4">
                      ...
                      <div className="ml-4">
                        <h3 className="text-lg font-bold text-gray-900">Wellness Score</h3>
                        <p className="text-sm text-gray-600">Overall health rating</p>
                      </div>
                    </div>
          ...
          {activeSection === 'schedule' && (
            ...
            >
              ...
              <div className="bg-gradient-to-br from-white to-green-50/30 rounded-3xl p-8 shadow-xl border border-green-200/50">
                <div className="text-center mb-8">
                  <h2 className="text-4xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent mb-4">
                    🌿 My Schedule & Treatment Plans
                  </h2>
                  <p className="text-slate-600 text-lg">Your appointments and personalized healing protocols</p>
                </div>

                {(() => {
                  const savedProfile = localStorage.getItem('patientProfile');
                  const patientName = savedProfile ? JSON.parse(savedProfile).name : getDisplayName();

                  // Get treatment plans created by doctors
                  const treatments = treatmentService.getPatientTreatments(patientName);

                  // Get regular appointments from all doctors
                  const allAppointments = appointmentManager.getAllAppointments();
                  const patientAppointments = [];

                  Object.keys(allAppointments).forEach(doctorUsername => {
                    const doctorAppointments = allAppointments[doctorUsername].filter(apt =>
                      apt.patientName.toLowerCase() === patientName.toLowerCase()
                    );
                    patientAppointments.push(...doctorAppointments);
                  });

                  const hasContent = treatments.length > 0 || patientAppointments.length > 0;
                  ...
                  if (hasContent) {
                    return (
                      <div className="space-y-6">
                        ...
                        {treatments.map((treatment, index) => (
                          <div key={treatment.id} className="bg-white rounded-2xl p-8 border border-green-200/50 shadow-lg hover:shadow-xl transition-all duration-300">
                            <div className="flex items-start justify-between mb-6">
                              <div className="flex items-start space-x-4">
                                <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl flex items-center justify-center">
                                  <span className="text-2xl">
                                    {treatment.treatmentType.includes('Panchakarma') ? '🧘' :
                                      treatment.treatmentType.includes('Herbal') ? '🌿' :
                                        treatment.treatmentType.includes('Yoga') ? '⚡' : '🌱'}
                                  </span>
                                </div>
                                <div>
                                  <h3 className="text-2xl font-bold text-gray-900 mb-1">{treatment.treatmentType}</h3>
                                  <div className="flex items-center space-x-4 text-sm text-gray-600">
                                    <span className="flex items-center">
                                      <span className="mr-1">👨⚕️</span>
                                      Dr. {treatment.doctorName || 'Your Doctor'}
                                    </span>
                                    <span className="flex items-center">
                            ...
                            <div className="bg-gradient-to-r from-green-50/50 to-emerald-50/50 rounded-xl p-6 border border-green-100">
                              <div className="flex items-center mb-4">
                                ...
                                <h4 className="text-lg font-bold text-gray-900">Your Personalized Treatment Protocol</h4>
                              </div>
                            ...
                            {treatment.appointmentDate && treatment.appointmentTime && (
                                ...
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <div className="flex items-center space-x-4 text-sm text-gray-700">
                                      <span className="flex items-center">
                                        <span className="mr-1">📆</span>
                                        {new Date(treatment.appointmentDate).toLocaleDateString('en-US', {
                                          weekday: 'long',
                                          year: 'numeric',
                                          month: 'long',
                                          day: 'numeric'
                        ...
                        {patientAppointments.map((appointment, index) => (
                          <div key={appointment.id} className="bg-white rounded-2xl p-8 border border-blue-200/50 shadow-lg hover:shadow-xl transition-all duration-300">
                            <div className="flex items-start justify-between mb-6">
                              <div className="flex items-start space-x-4">
                                <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center">
                                  <span className="text-2xl">📅</span>
                                ...
                                <div>
                                  <h3 className="text-2xl font-bold text-gray-900 mb-1">Consultation Appointment</h3>
                                  <div className="flex items-center space-x-4 text-sm text-gray-600">
                                    <span className="flex items-center">
                                      <span className="mr-1">📅</span>
                              ...
                              <div className="flex flex-col items-end space-y-2">
                                <span className={`px-4 py-2 rounded-full text-sm font-semibold border ${appointment.status === 'confirmed' ? 'bg-green-100 text-green-800 border-green-200' :
                    ...
                    return (
                      <div className="text-center py-16">
                        ...
                        <h3 className="text-2xl font-bold text-gray-700 mb-3">Your Healing Journey Awaits</h3>
                        ...
                        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 max-w-lg mx-auto border border-green-200">
                          <h4 className="font-semibold text-gray-800 mb-2">👨⚕️ What to Expect:</h4>
                          <ul className="text-sm text-gray-600 space-y-1 text-left">
                            <li>• Comprehensive dosha analysis</li>
                            <li>• Customized Panchakarma protocols</li>
        ...
        {activeSection === 'dashboard' && (
          <div className="space-y-8">
            ...
            <motion.div
              ...
              className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 md:gap-10 lg:gap-12 max-w-7xl mx-auto px-4"
            >
              {[
                {
                  id: 'book-appointment',
                ...
                <motion.div
                  ...
                  className={`group relative cursor-pointer overflow-hidden rounded-3xl bg-white/95 backdrop-blur-xl border border-white/20 shadow-xl hover:shadow-2xl p-8 h-72 w-full transition-all duration-500 transform-gpu focus:outline-none focus:ring-4 focus:ring-green-200/50 lg:col-span-2 ${item.id === 'health' ? 'lg:col-start-2' : ''} ${item.id === 'feedback' ? 'lg:col-start-4' : ''}`}
                >
                  ...
                  <div className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${item.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 blur-sm`} />
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className="relative">
                        ...
                        >
                          ...
                          {/* Icon glow effect */}
                          <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${item.iconBg} opacity-50 blur-md group-hover:opacity-70 transition-opacity duration-300`} />
                        </motion.div>
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <div className="mt-6">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: '100%' }}
                          transition={{ delay: item.delay + 0.8, duration: 1.2, ease: "easeOut" }}
